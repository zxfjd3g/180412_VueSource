...
## debug调试
    ...
    3. 加断点的时机/方式
      ...
      b. 在程序运行前: 在浏览器中添加断点(js是直接运行的)
      c. 在程序运行过程中加断点
    4. debug的5个操作
      a. 恢复程序的执行(执行完/暂停在下一个断点处)
      b. 执行完当前语句, 进入下一条语句(可能会暂停在中间的某个断点)
      c. 跳入当前函数调用中
      d. 跳出当前函数的执行(执行完当前函数后面所有的)
      e. 让断点暂时失效(后面需要时, 可以再次恢复使用)
...
## 3. 模板解析(compile.js)
    ...
    3.解析表达式文本节点: node.textContent = value
      	1). 根据正则对象得到匹配出的表达式字符串: 子匹配/RegExp.$1
      	2). 从data中取出表达式对应的属性值
      	3). 将属性值设置为文本节点的textContent
    4.事件指令解析: node.addEventListener(eventName, callback)
      	1). 从指令名中取出事件名
